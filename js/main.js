...
document.addEventListener("DOMContentLoaded", () => {
    ...
    links.forEach((link, index) => {
        ...
        card.style.setProperty("--link-glow", link.glow || "transparent");
        card.style.setProperty("--stagger-delay", `${100 + index * 75}ms`);
        ...
        copyButton.setAttribute("aria-label", `Copy ${link.name} link`);
        ...
        copyButton.addEventListener("click", async (event) => {
            ...
            try {
                ...
                console.error(`Failed to copy ${link.name} link:`, error);

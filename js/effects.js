    ...
    if (hasFinePointer && !reducedMotion) {
        ...
        window.addEventListener(
            ...
            (event) => {
                ...
                pointerTargetY =
                    (event.clientY / window.innerHeight - 0.5) * 24;
    ...
    */
    ...
    function updateParallax() {
        ...
        const mobileMultiplier =
            window.innerWidth <= 700 ? 0.82 : 1;

        if (!reducedMotion) {
            root.style.setProperty(
                "--parallax-back-y",
                `${scrollCurrent * -0.055 * mobileMultiplier}px`
            );

            root.style.setProperty(
                "--parallax-middle-y",
                `${scrollCurrent * -0.11 * mobileMultiplier}px`
            );

            root.style.setProperty(
                "--parallax-front-y",
                `${scrollCurrent * -0.19 * mobileMultiplier}px`
            );
